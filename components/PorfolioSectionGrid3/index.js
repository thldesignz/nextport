import React from 'react'
import Link from 'next/link'

const PorfolioSectionGrid3 = ( props ) => {
    return(
        <div className={`wpo-protfolio-area-3 section-padding ${props.protfclass}`}>
            <div className="container">
                <div className="row">
                    <div className="col col-xs-12 sortable-gallery">
                        <div className="gallery-container row">
                            <div className="col-lg-6 col-md-6 col-sm-6 custom-grid">
                                <div className="protfolio-l">
                                    <div className="wpo-protfolio-single">
                                        <div className="wpo-protfolio-img">
                                            <img src='images/protfolio/img-9.jpg' alt=""/>
                                        </div>
                                        <div className="wpo-protfolio-text">
                                            <h2><Link href="/ProjectDetails"><a>Minimalism</a></Link></h2>
                                            <span>Illustration . Art Direction</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="protfolio-l ">
                                    <div className="wpo-protfolio-single">
                                        <div className="wpo-protfolio-img">
                                             <img src='images/protfolio/img-10.jpg' alt=""/>
                                        </div>
                                        <div className="wpo-protfolio-text">
                                            <h2><Link href="/ProjectDetails"><a>Abstract Art</a></Link></h2>
                                            <span>Illustration . Art Direction</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-lg-6 col-md-6 col-sm-6 custom-grid">
                                <div>
                                    <div className="wpo-protfolio-single">
                                        <div className="wpo-protfolio-img">
                                            <img src='images/protfolio/img-11.jpg' alt=""/>
                                        </div>
                                        <div className="wpo-protfolio-text">
                                            <h2><Link href="/ProjectDetails"><a>Modern BG</a></Link></h2>
                                            <span>Illustration . Art Direction</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="">
                                    <div className="wpo-protfolio-single">
                                        <div className="wpo-protfolio-img">
                                            <img src='images/protfolio/img-9.jpg' alt=""/>
                                        </div>
                                        <div className="wpo-protfolio-text">
                                            <h2><Link href="/ProjectDetails"><a>3D Project</a></Link></h2>
                                            <span>Illustration . Art Direction</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="protfolio-btn">
                            <Link href="/PorfolioGrid3" className="theme-btn-s2"><a>Load More</a></Link>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default PorfolioSectionGrid3;